...
class FakeMicrophoneLoop:
    def __init__(self, audio_file, audio_device, rate=16000, channels=1):
        command = ["ffmpeg"]
        command += ['-re'] # Read input at native frame rate
        ...
        self.device = open(audio_device, 'w') #os.open(audio_device, os.O_WRONLY | os.O_SYNC)
